...
const LISTA_BARBEROS = [
  { nombre: 'Charlie', foto: '/charlie.png' },
  { nombre: 'Barbero 2', foto: 'https://ui-avatars.com/api/?name=Barbero+2&background=random' },
  { nombre: 'Barbero 3', foto: 'https://ui-avatars.com/api/?name=Barbero+3&background=random' },
  { nombre: 'Barbero 4', foto: 'https://ui-avatars.com/api/?name=Barbero+4&background=random' }
];
      ...
      <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-md border border-gray-100 relative overflow-hidden">
        {ticket ? (
          <div className="animate-in zoom-in duration-500">
            ...
            <button onClick={nuevaReserva} className="w-full bg-gray-900 text-white font-black py-4 rounded-xl hover:bg-gray-800 transition-all uppercase tracking-widest text-xs active:scale-95">
              Hacer otra reserva
            </button>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" name="cliente_nombre" value={formData.cliente_nombre} onChange={handleChange} required className="w-full px-4 py-2 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-600 outline-none text-black font-bold placeholder:text-gray-400" placeholder="Ej. Lucas" />
              </div>
              <div>
                <label className="block text-gray-900 text-xs font-black uppercase mb-1 ml-1">WhatsApp</label>
                <input type="tel" name="cliente_telefono" value={formData.cliente_telefono} onChange={handleChange} required className="w-full px-4 py-2 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-600 outline-none text-black font-bold placeholder:text-gray-400" placeholder="7000-0000" />
              </div>
              <div>
                ...
                <div className="flex gap-4 justify-center">
                  {LISTA_BARBEROS.map((b) => (
                    <div key={b.nombre} onClick={() => seleccionarBarbero(b.nombre)} className={`flex flex-col items-center cursor-pointer p-2 rounded-2xl border-2 transition-all ${formData.barbero === b.nombre ? 'border-blue-600 bg-blue-50 shadow-md scale-105' : 'border-gray-100 hover:bg-gray-50'}`}>
                      <img src={b.foto} className="w-16 h-16 rounded-full object-cover mb-2 border-2 border-white shadow-sm" alt={`Foto de ${b.nombre}`} />
              ...
              <div>
                ...
                <input type="date" name="fecha" value={formData.fecha} onChange={handleChange} required className="w-full px-4 py-2 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-600 outline-none bg-white text-black font-bold" />
              </div>
              {formData.fecha && (
